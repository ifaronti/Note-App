...
import { parse_query } from "@/components/notes/helper";
...
export function icon_theme(current: string, theme: string, item: string) {
    let color = theme
    if (theme == 'system' && window.matchMedia('preferred-color-scheme:light').matches) {
        color = 'light'
    }
    else {
        color = 'dark'
    }
    return current == item ? '#335CFF' : color === 'dark'? '#fff':'#0E121B'
}
    ...
    return (
        <div className="w-full flex flex-col">
            <button onClick={all_notes} className={`flex px-3 w-full h-10 ${current_pane === "Home"? "bg-auth_page text-text9 rounded-lg":"bg-none text-text7"} justify-between items-center`}>
                ...
                {current_pane === "Home" && <span>{chevy_home}</span>}
            </button>
            ...
            <button onClick={archived} className={`flex px-3 h-10 w-full ${current_pane === "Archived"? "bg-auth_page text-text9 rounded-lg":"bg-none text-text7"} justify-between items-center`}>
                ...
                {current_pane === "Archived" && <span>{chevy_Archived}</span>}
            </button>
        </div>
    )
}